...
The fast-moving sources refresh automatically: `python pipeline/build.py --fetch` brings the YCS monthly report, the DfE exclusions release and the annual Youth Justice Statistics up to date before building, and a scheduled workflow does the same on the 15th of each month, opening a pull request when the data changes. When the refresh opens a pull request it also cuts a raw-data bundle matching that data as a draft release, and merging the pull request publishes it, so the newest published bundle always reproduces `main`. When a manual source (ONS Census, IMD, Home Office, StatsWales) is refreshed locally, the procedure is still by hand: re-run the build, run `python pipeline/make_raw_bundle.py` and publish the dated release it prints, then note the refresh in `docs/data-sources.md`. Each cut is a new release; a published bundle is never rewritten in place, because readers cite it to verify the pipeline against its inputs.

Four workflows guard this. `ci.yml` runs the fast tests on every push and pull request. `verify.yml` runs weekly, and whenever a raw-data release is published, downloading the newest published bundle and building from it offline: the result must reproduce the committed `data/processed/` byte for byte, which is the reproducibility claim tested rather than asserted. `refresh.yml` runs monthly, building with `--fetch`, and when a source has published new data opens a pull request, cuts a matching bundle as a draft release, and dispatches `ci.yml` onto the pull request branch, since a pull request opened by a workflow raises no event that would run it otherwise; the dispatched run reports a `CI (dispatched)` status onto the commit, which is how it appears on the pull request. `publish-bundle.yml` publishes that draft when the pull request merges, pointing its tag at the merge commit, which in turn triggers `verify.yml`.
