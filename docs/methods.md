...
**When it was found and fixed.** Found at 16:49 on 3 September 2026 while testing the browser CSV export, by inspecting the explorer payload for a cell already known to be suppressed and finding its rate present. Fixed the same hour: `Step` now carries an `after_suppression` marker, the two derived steps are marked with it, and `build.py` runs the suppression stage between the ordinary steps and the derived ones. The corrected data was deployed before this entry was written.

**The rule change that followed.** No change to the disclosure rules themselves; the rules were right and the pipeline had not applied them in the correct order. The guard was extended instead. The existing test checks suppressed records for stray values, which cannot see this defect, because these records did not claim to be suppressed. `tests/test_suppression.py` now also cross-checks every derived output against the parent file: a cell suppressed in `context_indicators.json` that carries a value in an explorer payload or a CSV export fails the suite. `tests/test_build.py` asserts that every step deriving from suppressed data is marked to run after that stage, and that those steps sit contiguously at the end of the plan. The first of those tests was written before the fix and reproduced the defect at 141 cells, which is how the count above was established.
