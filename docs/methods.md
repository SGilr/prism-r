...
HMI Prisons' annual survey Children in custody 2025-26 (22 September 2026) is quoted on the national picture page for the background and experience of children on remand, and for the ethnic composition of the surveyed custody population. It is quoted, not ingested. It is a self-report survey of 315 respondents at five inspections between April 2025 and March 2026; legal status is asked as "on remand or waiting to be sentenced", which includes convicted children awaiting sentence and 18-year-olds, so its 41% unsentenced is on a different basis from the YCS remand stock and is not reconciled with it. It carries no home geography and no community denominator, so nothing in it can enter the Relative Rate Index or the explorer, and per-establishment counts derived from its published percentages would fall below the disclosure threshold in several cells. Where the page quotes a difference between groups, it carries HMIP's own significance flag at the 1% level; differences HMIP does not flag are reported as differences in the survey, not as findings.
...
### 23 September 2026: the suppression audit published the values it suppressed

**What the defect was.** The suppression audit, `suppression_audit.json`, records every disclosure-control decision so that a reader can see the rule applied. Each entry for a suppressed cell also carried the cell's value before suppression, in an `original_count` field, and each primary entry repeated it in words ("count 5 below threshold 6"). The audit was committed to the repository and served from the site, so every suppressed count it described was published beside the note saying it had been suppressed. The suppression held in the data files and was undone by the file documenting it.

**Which file and which cells.** `data/processed/suppression_audit.json`, and the same file served at `/data/suppression_audit.json`. At the time of the fix it disclosed 203 suppressed values: 143 from `context_indicators.json`, 54 from `custody_monthly.json` and 6 from `remand_outcomes.json`. 89 were primary suppressions, counts of 1 to 5 (59, 27 and 3 respectively); the other 114 were secondary. Earlier versions of the audit disclosed a partly different set: 147 values from 18 May 2026, 201 once the monthly custody data was added on 3 September 2026, and 203 after the Welsh looked-after children re-ingest on 5 September 2026, which replaced 20 values for the year ending March 2024 with values for the year ending March 2025. 223 distinct values were disclosed in all. The 154 entries for cells suppressed at source by the publisher carried no value, so no publisher's own withholding was breached.

The 141 context-indicator cells in the audit before 5 September are the same 141 cells recorded in the second 3 September entry below, and they include the 30 stop and search cells of the first. Both of those entries describe their exposure too narrowly: the same values were served from the site through the audit from 18 May 2026, not only for the periods those entries give.

**How long it was present.** The field was written by the suppression module from its first commit, `86001f4`, on 17 May 2026. The audit was first committed to the public repository on 18 May 2026 in commit `30f4ab3`, and was served from the site from the deployment made that day. It was corrected in commit `931beaa` and redeployed at 18:38 on 23 September 2026: 128 days. Cloudflare Pages keeps every earlier deployment reachable at its own `pages.dev` preview address, and all 27 earlier deployments were still serving the uncorrected audit after the fix. They were deleted through the Cloudflare API the same evening. Two of the 27 addresses stopped resolving at once; the other 25 were still being served by Cloudflare at 19:10 on 23 September 2026, although the API reports the deployments as deleted. This remains open, and this entry will be updated when the addresses stop resolving. The values remain in the repository's history before commit `931beaa`, which has not been rewritten. The Internet Archive's CDX index was checked on 23 September 2026 and holds no capture of `prism-r.howpreventionworks.com`, of `/data/suppression_audit.json`, or of any `prism-r.pages.dev` deployment preview.

**How it was found.** On 23 September 2026, while checking a stale sentence in `docs/disclosure-control.md` that said the audit file had not yet been written. Nobody outside the project reported it. The structural guard added on 3 September 2026 checks every processed output for a suppressed record that keeps a recoverable field, but it skipped `suppression_audit.json` by name, so the one published file that described every suppressed cell was the one file it did not read.

**When it was fixed.** The same day, in commit `931beaa`. Entries for suppressed cells now record the cell, its group, the rule and the reason, with `original_count` null; a primary entry says the count was between 1 and 5. A rate-threshold entry keeps its count, which is released, unless the cell is also suppressed.

**What followed.** Two tests in `tests/test_suppression.py` hold the change: one runs the rules and fails if any suppressed entry carries a value, and one reads the committed audit and fails on the same condition, so the published file is checked as well as the code that writes it. The live file was checked after deployment: its SHA-256 matches the repository's, and no entry with `resulting_state` "suppressed" has a non-null `original_count`. Every file in the corrected build was hashed against the live site and all matched. The earlier deployment previews were deleted, with the caveat above.

**Assessment.** Every disclosed value was checked against the publishers' own files in the raw data bundle. All 223 are published there: 171 as a single published cell (DfE and StatsWales looked-after children by local authority and ethnic group, YCS monthly custody tables 1.2 and 1.4, and YJB Table 6.1), and 52 as the sum of published rows in the Home Office stop and search and arrests open data tables. PRISM-R suppresses these counts as an additional layer on figures the DfE, StatsWales, the YCS, the YJB and the Home Office release themselves, so the defect disclosed nothing that was not already public at source. It is recorded in full regardless. It is the third instance of the same failure, a suppression lost in a file derived from the one where it was applied, and this time the derived file was the audit written to show that suppression had been done.
